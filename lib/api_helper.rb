class Java::OrgMitre::ApiHelper
  ...
  def build_hash(parent, args = {})
    args[:handled_relationship] = false if args[:handled_relationship].nil?
    self.annotate_class!
      ...
      if child.kind_of?(Array) || child.kind_of?(Java::JavaUtil::ArrayList)
        if child.length > 0
          hash[method] = (0...child.length).map {|i|
            self.handle_single_item(child[i], parent, {:field => method, :from => hash, :index => i}, args)
          }
        end
      else
        val = self.handle_single_item(child, parent, {:field => method, :from => hash}, args)
        hash[method] = val unless val.nil?
      end
    end

    if self.respond_to?(:value) && !self.value.nil?
      hash['value'] = self.handle_single_item(self.value, parent, {:field => 'value', :from => hash}, args)
    end

    return hash
  end

  def handle_single_item(child, parent, proxy_args, args)
    args[:handled_relationship] = false if args[:handled_relationship].nil?
    if child.nil?
      return nil
      ...
      if child.idref.present?
        add_raw_relationship(parent, child, proxy_args[:field]) unless args[:handled_relationship]
        return {:idref => handle_single_item(child.idref, parent, {}, args)}
      else
        child_args = args.clone
        child_args.delete(:handled_relationship)
        child.save(child_args)
        add_raw_relationship(parent, child, proxy_args[:field]) unless args[:handled_relationship]
        return {:idref => handle_single_item(child.id, parent, {}, args)}
      end
    elsif child.respond_to?(:build_hash)
      return child.build_hash(parent, args)
